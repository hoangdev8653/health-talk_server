...
const createPostcard = async ({ title, decription, video_url }) => {
    try {
        return await db.Postcards.create({ title, decription, video_url });
    } catch (error) {
        console.log(error)
    }
}
...
const updatePostcard = async ({ id, title, decription, video_url }) => {
    try {
        ...
        return await db.Postcards.update({ title, decription, video_url }, { where: { id } });
    } catch (error) {
        console.log(error)
    }
}
